...
const Update = ({ recipe, content, user, update, setUpdate }) => {
  ...
  const [subtitle, onChangeSubtitle, setSubtitle] = useInput<
    string | undefined
  >("");
  const [url, onChangeUrl, setUrl] = useInput<string | undefined>("");
  const [category1, onChangeCategory1, setCategory1] = useInput<
    string | undefined
  >("메인반찬");
  const [category2, onChangeCategory2, setCategory2] = useInput<
    string | undefined
  >("일상");
  const [category3, onChangeCategory3, setCategory3] = useInput<
    string | undefined
  >("볶음");
  const [category4, onChangeCategory4, setCategory4] = useInput<
    string | undefined
  >("소고기");
  const [
    ingredients,
  ...
  const { mutate: addRecipe_Mutate, isLoading: addRecipeLoading } = useMutation(
    updateRecipe,
    {
      onSuccess: (data) => {
  ...
  const onSubmit = (e) => {
    ...
    Swal.fire("Success!", "성공적으로 수정 되었습니다!", "success");
  };
    ...
    <>
      ...
      <Form onSubmit={onSubmit} style={{ textAlign: "left" }}>
        <WriteContainer1
          ...
          onChangeCategory1={onChangeCategory1}
          onChangeCategory2={onChangeCategory2}
          onChangeCategory3={onChangeCategory3}
          onChangeCategory4={onChangeCategory4}
