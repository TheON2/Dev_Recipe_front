...
const recipeSlice = createSlice({
  ...
  reducers: {
    ...
    ADD_COMMENT: (
      state: RecipesState,
      action: PayloadAction<{ recipeId: string; comment: Comment }>
    ) => {
      if (state.recipes) {
        ...
        if (recipe) {
          recipe.comment = recipe.comment
            ? [...recipe.comment, action.payload.comment]
    ...
    DELETE_COMMENT: (
      state: RecipesState,
      action: PayloadAction<{ recipeId: string; commentId: string }>
    ) => {
      if (state.recipes) {
        const recipe = state.recipes.find(
          (r) => r.id === action.payload.recipeId
        ...
        if (recipe && recipe.comment) {
          recipe.comment = recipe.comment.filter(
